...
from utils import data_split, equalize_splits
...
def evaluate_on_benchmark(
    ...
    for i in range(nsplit):
        if isinstance(model, TransformerModel):
            ...
            batch = Prior.graphs2batch([data], max_nfeat=original_nfeat, neig=neig, node_deg=node_deg, attention=attention, split_name="random", attention_nhead=4)
...
if __name__ == "__main__":
    ...
    model = torch.load(output_folder / "model.pth", map_location=device)
