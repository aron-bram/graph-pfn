...
from torch_geometric.transforms import (AddLaplacianEigenvectorPE,
                                        AddRandomWalkPE)
from torch_geometric.utils import (barabasi_albert_graph, coalesce, degree,
                                   erdos_renyi_graph, is_undirected,
                                   to_undirected)
from utils import Output, Sampler, setup_logging
...
class Prior:
    ...
    def graphs2batch(
        ...
        split_pos = Prior.generate_train_test_split(
            split_name=split_name, nnode=len(graphs[0].y)
        )
    ...
    def sample_batch(
        ...
        **kwargs,  # kwargs needed for train()
        ...
        batch = self.graphs2batch(
            graphs,
            self.nfeat.max_val,
            neig=neig,
            node_deg=node_deg,
            split_name=self.split_name.sample(),
            attention=attention,
